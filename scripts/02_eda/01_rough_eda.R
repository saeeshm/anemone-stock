...
# ==== Reading data ====
ceff <- read_csv('data/raw/catch_effort_infilled.csv') %>% 
  # Column indicating if data was interpolated or not
  mutate(isintp = is.na(eff_hours))
field <- read_csv('data/raw/anemone_field_data.csv')
weight <- read_csv('data/raw/anemone_weight_data.csv')
sales <- read_csv('data/raw/fish_market_data.csv')
